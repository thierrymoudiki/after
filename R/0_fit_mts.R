                    ...
                    verbose = FALSE)
{
  ...
  current_error_norm <- norm(current_error, type = "F")
    ...
    if (col_sample < 1) # columns' subsampling
    {
        if (hidden_layer_bias == FALSE)
        {
          # 1 - col_sample < 1 && hidden_layer_bias == FALSE ------------------------------------------------------------------
    ...
    # 3 - col_sample == 1 && hidden_layer_bias == FALSE ------------------------------------------------------------------
...
# Fitting a constrained regression  model to multiple time series (with > 0 coeffs)
                             ...
                             seed = 1)
{
  ...
  fit_obj <- lapply(1:nb_series,
                    function (i) {data.train <- cbind(y = centered_y[, i], xreg);
                                  dtrain <- xgb.DMatrix(data.train, label = data.train[, 1]);
