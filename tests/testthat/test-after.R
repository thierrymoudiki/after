...
# compare with meanf and rwf + ns MTS
...
# 2 - multivariate time series test ----------------------------------------------------------

#x <- fpp::insurance

#test_that("test multivariate time series methods", {
  # xgboost remain to be tested
  ...
  # expect_equal(check_closeness(as.numeric(after::glmboostf(x)$preds[3, 1]), 13.19295 ), TRUE)
  # expect_equal(check_closeness(as.numeric(after::scnf(x)$preds[3, 1]), 15.63736), TRUE)
  # expect_equal(check_closeness(as.numeric(after::varf(x)$preds[3, 1]), 13.77753), TRUE)
  # expect_equal(check_closeness(as.numeric(after::varf(x, penalization = "l1")$preds[3, 1]), 13.64782), TRUE)
  # expect_equal(check_closeness(as.numeric(after::xgboostf(x)$preds[3, 1]), 13.53228 ), TRUE)
  # })
